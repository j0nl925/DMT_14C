...
class VESCTestCase(TestCase):
    def test_measurements_with_duty_cycle_ramp(self):
        vesc = VESC("COM4")
        ...
        # Go through duty cycle from 4% to 20% in 1% increments
        for i in range(4, 21):
            try:
                duty_cycle = i / 100.0
                ...
                # Get measurements from the VESC
                measurements = vesc.get_measurements()

                # Store the data point
                data_points.append([duty_cycle, measurements])
        ...
        filename = "motor_measurements.csv"
        header = ["Duty Cycle", "Measurements"]
...
# Create an instance of the VESC class with the appropriate port
vesc = VESC("COM4")  # Replace "COM4" with the actual port of your VESC

# Create a test suite with the VESCTestCase class
test_suite = TestLoader().loadTestsFromTestCase(VESCTestCase)
